function submitWithConfiguration(conf)
  ...
  try
    response = submitParts(conf, email, token, part_variable);
  catch
    e = lasterror();
    fprintf('\n!! Submission failed: %s\n', e.message);
    fprintf('\n\nFunction: %s\nFileName: %s\nLineNumber: %d\n', ...
  ...
  elseif isfield(response, 'errorCode')
    ...
    showFeedback(part_variable, response);
...
function response = submitParts(conf, email, token, parts)
  ...
  submissionUrl_variable = submissionUrl();

  responseBody = getResponse(submissionUrl_variable, body);
