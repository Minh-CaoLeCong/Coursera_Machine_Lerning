...
X = [ones(m,1), X]; % Adding 1 as the first column in X
                    % m x (input_layer_size + 1)

a1 = X; % m x (input_layer_size + 1)
...
y_Vec = (1:num_labels) == y; % m x num_labels
% Anoter way to converting y
...
% % ----- Part 2: Implementing backpropogation for Theta_grad -----
...
%   yVector = (1:num_labels)'==y(i); % num_labels x 1
%   
%   % calculating delta values
%   delta3 = a3 - yVector; % num_labels x 1
%   delta2 = (Theta2' * delta3) .* [1; sigmoidGradient(z2)]; % (hidden_layer_size+1) x 1
%   delta2 = delta2(2:end); % hidden_layer_size x 1 % Removing delta2 for bias node  
%   
%   % delta_1 is not calculated because we do not associate error with the input  
%   
%   % CAPITAL delta update
%   Theta1_grad = Theta1_grad + (delta2 * a1'); % hidden_layer_size x (input_layer_size + 1)
%   Theta2_grad = Theta2_grad + (delta3 * a2'); % num_labels x (hidden_layer_size + 1)
% end
% 
% Theta1_grad = (1/m) * Theta1_grad; % hidden_layer_size x (input_layer_size + 1)
% Theta2_grad = (1/m) * Theta2_grad; % num_labels x (hidden_layer_size + 1)
...
% Calculating delta values
DELTA3 = A3 - y_Vec; % m x num_labels
DELTA2 = (DELTA3 * Theta2) .* [ones(size(Z2,1),1) sigmoidGradient(Z2)]; % m x (hidden_layer_size + 1)
DELTA2 = DELTA2(:,2:end); % m x hidden_layer_size % Removing delta2 for bias node

Theta1_grad = (1/m) * (DELTA2' * A1); % hidden_layer_size x (input_layer_size + 1)
Theta2_grad = (1/m) * (DELTA3' * A2); % num_labels x (hidden_layer_size + 1)

% -------------------------------------------------------------

% ---- Part 3: Using Regularization to compute cost function and grad ----
regularized = (lambda/(2*m)) * (sum(sum(Theta1(:,2:end).^2))...
                + sum(sum(Theta2(:,2:end).^2)));                %scalar

% Cost function with regularization
J = J + regularized; % scalar
